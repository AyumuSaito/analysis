...
rewrite [X in measurable_fun _ X](_ : _ =
  (fun x => if 0 <= x <= 1 then (\sum_(m < n.+1)
...
transitivity (lim (fun n => \int[m]_(x in setT) (f_ n x)%:E)).
...
rewrite [X in _ = _ * X](_ : _ = lim
    (fun n => \int[lebesgue_measure]_(x in `[a, b]) (f_ n x)%:E)); last first.
  rewrite -monotone_convergence//=.
...
rewrite -limeMl//.
  by congr (lim _); apply/funext => n /=; exact: integral_uniform_nnsfun.
