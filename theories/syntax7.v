...
| exp_real {R : realType} : R -> expD reals
| exp_pair : forall t1 t2, expD t1 -> expD t2 -> expD (pairs t1 t2)
| exp_var  : forall {l : context} (x : string), expD (nth units (map snd l) (seq.index x (map fst l)))
(* | exp_plus : expD -> expD -> expD *)
(* | val_unif : val *)
| exp_bernoulli {R : realType} : {nonneg R} -> expD (probs bools)
...
| exp_letin : forall y z, string -> expP y -> expP z -> expP z
...
Lemma eval_uniqP (l : context) (T : types) (e : expP T) 
...
Example pgm1 : expD (probs bools) := exp_norm (
  exp_letin "x" (exp_sample (@exp_bernoulli R (2 / 7%:R)%:nng)) (
  exp_letin "r" (exp_if (@exp_var [:: ("x", bools)] "x") (exp_return (@exp_real R 3%:R)) (exp_return (@exp_real R 10%:R))) (
  exp_letin "_" (exp_score (exp_poisson 4 (@exp_var [:: ("r", reals); ("x", bools)] "r"))) (
  exp_return (@exp_var [:: ("_", units); ("r", reals); ("x", bools)] "x"))))).
...
Example ev1 : @evalD R [::] _ pgm1 _ (measurable_fun_normalize kstaton_bus'' point).
Proof.
