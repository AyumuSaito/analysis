...
Definition exp0 : expD := Real 1.

Definition exp1 : expP := Sample (Bernoulli (2 / 7%:R)%:nng).

Definition exp2 : expP := Letin "x" (Sample (Bernoulli (2 / 7%:R)%:nng)) (Return (Var "x")).

Example exp3 : expD := Norm (
  Letin "x" (Sample (Bernoulli (2 / 7%:R)%:nng)) (
  Letin "r" (If (Var "x") (Return (Real 3%:R)) (Return (Real 10%:R))) (
  Letin "_" (Score (Poisson 4 (Var  "r"))) (
  Return (Var "x"))))).
...
| tc_norm G e d (A : measurableType d) :
  type_checkP G e A ->
  type_checkD G (Norm e) (probability A R)

| tc_var G v G' A : type_checkD (G ++ (v, A) :: G')%SEQ (Var v) A

with type_checkP : Env -> expP -> Type -> Prop :=
| tc_sample G e d (A : measurableType d) : type_checkD G e (probability A R) ->
  type_checkP G (Sample e) A
| tc_return G e A : type_checkD G e A -> type_checkP G (Return e) A
| tc_score G e : type_checkD G e R -> type_checkP G (Score e) unit
...
(* 
Inductive valueD : forall {dG dT} {G : measurableType dG} {T : measurableType dT}, Set :=
...
(* Inductive valueD : expD -> Prop :=
| VUnit : valueD Unit
| VBool : forall b, valueD (Bool b)
| VReal : forall r, valueD (Real r)
| VPair : forall v1 v2,
  valueD v1 -> valueD v2 ->
  valueD (Pair v1 v2)
with valueP : expP -> Prop :=
| VIte : forall e1 e2 e3, valueP (If e1 e2 e3)
| VRet : forall e, valueP (Return e)
. *)
...
Inductive EvalD : Env -> expD -> ValD -> Prop :=
| EUnit : forall E, EvalD E Unit VUnit
| EBool : forall E b, EvalD E (Bool b) (VBool b)
| EReal : forall E r, EvalD E (Real r) (VReal r)
.
...
Fixpoint Equiv (val : ValD) (ty : Ty) : Prop :=
  match ty with
  | TyUnit => val = VUnit
  | TyBool => exists b, val = VBool b
  | TyReal => exists r, val = VReal r
  | TyPair A B => exists v1 v2, val = VPair v1 v2
  end.
End equiv.
...
Inductive StepD : Env -> expD -> expD -> Prop :=
(* | STUnit : forall E,
  StepD E Unit Unit

| EBool : forall E b, 
  StepD E (Bool b) (Bool b)

| STReal : forall E r, 
  StepD E (Real r) (Real r) *)

| STPlusl : forall E e1 e1' e2,
  StepD E e1 e1' ->
  StepD E (Plus e1 e2) (Plus e1' e2) 

| STPlusr : forall E v1 e2 e2',
  valueD v1 ->
  StepD E e2 e2' ->
  StepD E (Plus v1 e2) (Plus v1 e2')

| STPlus : forall E r1 r2,
  StepD E (Plus (Real r1) (Real r2)) (Real (r1 + r2))

| STBool : forall E b,
  StepD E (Bool b) (Bool b)

| STPairl : forall E e1 e1' e2,
  StepD E e1 e1' ->
  StepD E (Pair e1 e2) (Pair e1' e2)

| STPairr : forall E v1 e2 e2',
  valueD v1 ->
  StepD E e2 e2' ->
  StepD E (Pair v1 e2) (Pair v1 e2')

| STPair : forall E v1 v2,
  valueD v1 -> valueD v2 ->
  StepD E (Pair v1 v2) (Pair v1 v2)

| STNorm : forall E e1 e1',
  StepP E e1 e1' ->
  StepD E (Norm e1) (Norm e1')
with StepP : Env -> expP -> expP -> Prop :=
| STRet : forall E e1 e1',
  StepD E e1 e1' ->
  StepP E (Return e1) (Return e1')
| STSample : forall E e1 e1',
  StepD E e1 e1' ->
  StepP E (Sample e1) (Sample e1')
| STScore : forall E e e',
  StepD E e e' ->
  StepP E (Score e) (Score e')
| STLetinl : forall E x e1 e1' e2,
  StepP E e1 e1' ->
  StepP E (Letin x e1 e2) (Letin x e1' e2)
| STLetinr : forall E x e1 e2 e2',
  valueP e1 ->
  StepP E e2 e2' ->
  StepP E (Letin x e1 e2) (Letin x e1 e2')
| STLetin : forall E x e1 e2,
  valueP e1 -> valueP e2 ->
  StepP E (Letin x e1 e2) (Letin x e1 e2)
.

Scheme stepD_mut_ind := Induction for StepD Sort Prop
with stepP_mut_ind := Induction for StepP Sort Prop.
...
Notation multistep := (multi (StepD [::])).

Definition ex1 := Norm (Return (Plus (Real 1) (Real 2))).
Definition ex1' := Norm (Return (Real 3)).
...
Theorem progressD : forall e T,
  type_checkD [::] e T ->
  valueD e \/ exists e', StepD [::] e e'.
Proof.
move=> e T He.
have := (@stepD_mut_ind
  (fun E e0 e1 (h : StepD E e0 e1) => forall e T, valueD e \/ exists e', StepD [::] e e')
  (fun E e0 e1 (h : StepP E e0 e1) => forall e T, valueP e \/ exists e', StepP [::] e e')
  _ _ _ _ _ _ _ _ _ _ _ _ _ _).

remember [::] as G.
(* elim: He HeqG. *)
generalize dependent HeqG.
induction He => HeqG; subst.
- left. apply/VUnit.
- left. apply/VBool.
- left. apply/VReal.
- 
...
Theorem progressP : forall e T,
  type_checkP [::] e T ->
  valueP e \/ exists e', StepP [::] e e'.
Proof.
move=> e T He.
have := (@stepP_mut_ind
  (fun E e0 e1 (h : StepD E e0 e1) => forall e T, valueD e \/ exists e', StepD [::] e e')
...
Theorem preserveD e e' T :
  type_checkD [::] e T ->
  StepD [::] e e' ->
  type_checkD [::] e' T.
Proof.
move=> HT.
have := (@type_checkD_mut_ind
  (fun E e T (h : type_checkD E e T) => 
  forall e', StepD [::] e e' -> type_checkD [::] e' T)
  (fun E e T (h : type_checkP E e T) => 
  forall e', StepP [::] e e' -> type_checkP [::] e' T)
  _ _ _ _ _ _ _ _ _ _ _ [::]).
remember [::] as G.
generalize dependent HeqG.
generalize dependent e'.
induction HT => e' HeqG IH He'; inversion He' => //...
- apply/tc_bool.
- 
  + 
    apply/tc_pair.
...
Definition ex2 := Norm (Letin "x" (Sample (Bernoulli (2 / 7%:R)%:nng)) (Return (Var "x"))).



(*
Inductive StepD : forall dG (G : measurableType dG) dT (T : measurableType dT) (E : Env) (e : expD), expD -> Prop :=
| EBool : forall dG G E b, 
  @StepD dG G _ mbool E (exp_bool b) (@VBool _ _ G _ (exist _ (cst b) (kb b)))
| EUnit : forall dG G E, 
  @StepD dG G _ munit E (exp_unit) (@VUnit _ _ G _ (exist _ (cst tt) ktt)) *)
(* | EPair : forall dG G dA A dB B E e1 e2 v1 v2,
  @StepD dG G dA A E e1 v1 ->
  @StepD dG G dB B E e2 v2 ->
  @StepD dG G _ _ E (exp_pair e1 e2) (VPair v1 v2) *)
(* | EVar2 : forall (E : Env) x dX dY (X : measurableType dX) (Y : measurableType dY),
  let i := seq.index x (map fst E) in
  StepD E (exp_var x) (@VId _ _ _ _ _ _ [the measurableType _ of (X * Y)%type] (projT2 (typ2 X Y i)) (var_i_of2 i.+1)) *)
.
(* where "[G |- a ==> v]" := (Step G a v). *)
...
Step vm_compute in prod_meas (existT _ _ munit :: [::]).

Step vm_compute in size (existT _ _ munit :: [::]).
...
(*
valueD ::= measurable function (evalD)
        | kernel (evalP)
